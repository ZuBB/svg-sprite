...
import {
  isFunction, isObject, isPlainObject, merge, trimStart, zipObject
} from 'lodash-es';
import pretty from 'prettysize';
import File from 'vinyl';
import { ArgumentError } from './svg-sprite/errors.mjs';
import { SVGSpriteLayouter } from './svg-sprite/layouter.mjs';
import svgo from './svg-sprite/utils/svgo-transform.mjs';
import SVGSpriterConfig from './svg-sprite/config.mjs';
import Queue from './svg-sprite/queue.mjs';
...
function SVGSpriter(config) {
  this.config = new SVGSpriterConfig(config);
...
SVGSpriter.prototype._layout = function(config, cb) {
  const files = {};
  const layout = new SVGSpriteLayouter(this, config);
 ...
 */
export default function createSVGSpriterInstance(config = {}) {
